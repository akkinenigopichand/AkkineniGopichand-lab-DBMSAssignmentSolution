...
where t2.cat_id=category.cat_id group by category.cat_id order by category.cat_id;
...
select CUS_NAME,CUS_GENDER from Customer where cus_name like 'a%' or cus_name like '%a' ;
 ...
 select rat_id,Supplier.supp_id,Supplier.SUPP_NAME,t2.rat_ratstars,Type_of_Service from supplier 
 inner join 
( select Supplier_pricing .PRICING_ID,t1.rat_id,Supplier_pricing.supp_id ,t1.Type_of_Service,t1.RAT_RATSTARS from Supplier_pricing 
inner join 
(select `Order`.pricing_id,rating.rat_id,rating.RAT_RATSTARS,
case
...
else 'poor perfomance'
end as Type_of_Service
 from `Order` inner join rating
 where `Order`.ORD_ID = rating.ORD_ID ) as t1
